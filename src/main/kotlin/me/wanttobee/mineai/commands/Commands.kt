...
object Commands {
	...
	private fun sendEnvironmentVariables(source: CommandSourceStack) {
		...
		if (variables.isEmpty()) {
			source.sendSuccess(
				{ Component.literal("No environment variables found in ${EnvironmentVariables.MINEAI_FILE_NAME} or ${EnvironmentVariables.DOTENV_FILE_NAME}.").withStyle(ChatFormatting.YELLOW) },
				false
			)
			return
		}
		...
		source.sendSuccess(
			{ Component.literal("Merged environment variables (.env -> mineai.env -> runtime overrides):").withStyle(ChatFormatting.YELLOW) },
			false
		)
