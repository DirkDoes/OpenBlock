...
object OpenBlockCommands {
	private val executor: ExecutorService = Executors.newSingleThreadExecutor { runnable ->
		Thread(runnable, "openblock-commands").apply {
			isDaemon = true
		}
	}
	...
	internal fun showCurrentTarget(source: CommandSourceStack) {
	...
	private fun formatProviderStatus(provider: AiProvider, exception: Throwable?): Component {
	...
	private fun suggestToolNames(
		...
		return SharedSuggestionProvider.suggest(AiService.allTools().map { it.name }, builder)
	}
